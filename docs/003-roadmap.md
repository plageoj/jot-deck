...
* `u` / `Ctrl+z` で直近の削除から順に復元
* `g t` で削除スタック一覧表示、任意の項目を選択して復元

#### 未実装
* 汎用の操作履歴 Undo/Redo（`001-keybindings.md` §4.4）: 現状 `u` / `Ctrl+z` は削除の復元のみが対象。作成・テキスト編集・並び替えを対象に含めるセッション内 Undo スタックと、その Redo（`Ctrl+r` / `Ctrl+Shift+z`）は未実装
