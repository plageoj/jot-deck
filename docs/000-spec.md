...
* 初回起動時は、操作方法を実際のキー操作で学べるインタラクティブチュートリアルの起動を促す。任意のタイミングで再呼び出しも可能（詳細は `007-tutorial.md`）。
...
* インタラクティブチュートリアル（初回起動時の起動促し＋任意呼び出し、ハンズオン形式。`007-tutorial.md`）
* 論理削除と復元機能
* 30日後の物理削除
...
* `006-release.md` - リリース戦略（Production / Preview チャンネル、Tauri updater）
* `007-tutorial.md` - チュートリアル機能（初回起動・任意呼び出し）
