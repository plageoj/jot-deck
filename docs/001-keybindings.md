...
stateDiagram-v2
    [*] --> Column
    Column --> Card : j / k / Enter
    Card --> Column : Escape
    Card --> Edit : i / a / Enter
    Edit --> Card : Escape / :wq / Ctrl+Enter
    Column --> Command : Ctrl+Shift+p / F1
...
| キー | 動作 |
|:---|:---|
| `H` / `Shift+←` | カラムを左へ移動（並び替え） |
| `L` / `Shift+→` | カラムを右へ移動（並び替え） |
...
| `h` / `←` | 左のカラムへ移動（同じ行位置を維持） |
| `l` / `→` | 右のカラムへ移動（同じ行位置を維持） |
| `H` / `Shift+←` | カードを左のカラムへ移動 |
| `L` / `Shift+→` | カードを右のカラムへ移動 |
| `J` / `Shift+↓` | カードをカラム内で下へ移動（並び替え） |
| `K` / `Shift+↑` | カードをカラム内で上へ移動（並び替え） |
...
**対象操作:** カード / カラムの作成、削除（論理削除）、テキスト編集の保存、並び替え・カラム間移動。
...
> **実装状況:** 現時点で実装済みなのは削除の Undo（= ゴミ箱からの直近アイテム復元）のみ（`u` / `Ctrl+z`）。作成・編集・並び替えの Undo と、Redo（`Ctrl+r` / `Ctrl+Shift+z`）は未実装（`docs/003-roadmap.md` 参照）。
